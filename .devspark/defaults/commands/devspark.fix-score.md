...
> **Script Resolution**: Before running `.devspark/scripts/powershell/fix-score-context.ps1 $ARGUMENTS -Json`, apply the 2-tier override check - if `.documentation/scripts/powershell/fix-score-context.ps1` (PowerShell) or `.documentation/scripts/bash/fix-score-context.sh` (Bash) exists on disk, run that file instead, preserving all arguments. Team overrides in `.documentation/scripts/` always take priority over `.devspark/scripts/`.
...
- Run `.devspark/scripts/powershell/fix-score-context.ps1 $ARGUMENTS -Json` once and parse its JSON output. Use it as the first-pass blocker map before reading broad file trees.
